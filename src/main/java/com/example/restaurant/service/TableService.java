...
public class TableService {
    ...
    @Transactional
    public void moveOrders(Long fromTableId, Long toTableId) {
        TableEntity fromTable = tableRepository.findById(fromTableId)
                .orElseThrow(() -> new RuntimeException("Kaynak masa bulunamadı."));
        TableEntity toTable = tableRepository.findById(toTableId)
                .orElseThrow(() -> new RuntimeException("Hedef masa bulunamadı."));

        List<OrderEntity> orders = orderRepository.findByTable_Id(fromTableId);
        for (OrderEntity order : orders) {
            order.setTable(toTable);
        }
        orderRepository.saveAll(orders);

        fromTable.setOccupied(false);
        toTable.setOccupied(true);
