...
function loadTables() {
    ...
    // Eğer admin panel değilse çalıştırma
    if (!document.getElementById('tableList')) {
        console.warn('Admin paneli dışında loadTables() çağrıldı, atlanıyor.');
        return;
    }
        ...
        .then(tables => {
            ...
            if (Array.isArray(tables) && tables.length > 0) {
                tables.forEach(table => {
                   ...
                   const masaUrl = `${baseUrl}/?masa=${table.tableNumber}`;
                    const qrId = `qr-${table.tableNumber}`;
                    const btnId = `btn-${table.tableNumber}`;
                    ...
                    tableList.append(`
                        <tr>
                           ...
                           <td>
                               <canvas id="qr-${table.tableNumber}" width="100" height="100"></canvas>
                               <br>
                               <button class="btn-small blue" id="btn-${table.tableNumber}">
                                   <i class="material-icons">file_download</i> QR İndir
                               </button>
                           </td>
                           <td>
                ...
                tableList.html('<tr><td colspan="4">Henüz masa bulunmamaktadır.</td></tr>');
            }
        })
        .catch(error => {
            console.error('Masalar yüklenirken hata:', error);
            $('#tableList').html('<tr><td colspan="4" class="red-text">Masalar yüklenirken bir hata oluştu!</td></tr>');
        });
}
...
function addProduct() {
	...
	console.log('Ürün ekleniyor:', {
		...
		imageFile: imageFile ? imageFile.name : 'Resim yok'
	});
...
$(document).ready(function() {
	...
	// Materialize Sidenav güvenli başlatma
	if ($('.sidenav').length) {
		$('.sidenav').sidenav();
	}
...
function loadAllData() {
	...
	loadTables();
	initializeSelects();
	loadProductListForRecommendations()
}
...
function displayProducts(products, container) {
	...
	if (!Array.isArray(products) || products.length === 0) {
		container.html('<div class="col s12 center-align"><p>Bu kategoride ürün bulunmamaktadır.</p></div>');
		return;
	}
	...
	products.forEach(product => {
		...
		// Resim URL'sini kontrol et ve düzenle
		let imageUrl = product.imageUrl;
		if (imageUrl) {
			// URL'yi düzenle
			if (imageUrl.startsWith('/')) {
				imageUrl = imageUrl.substring(1); // Başındaki / işaretini kaldır
			}
			
			// Eğer resimler/ ile başlıyorsa, olduğu gibi bırak
			if (!imageUrl.startsWith('resimler/')) {
			...
			<div class="col s12 m6 l4">
				<div class="card">
					<div class="card-image">
						${imageUrl ? `<img src="${imageUrl}" alt="${product.name}" onerror="this.onerror=null; this.src='resimler/default-product.jpg'; console.log('Resim yüklenemedi:', this.src)">` : ''}
		...
		.then(product => {
			...
			if (existingItem) {
				...
				cartItems.push({
					...
					price: product.price,
					quantity: 1,
					imageUrl: product.imageUrl || 'uploads/default-product.jpg'
				});
			}
...
function openOrderModal() {
    ...
    const tableSelectDiv = document.getElementById('tableSelect').parentElement;

    if (masaNo) {
        // Masa select ve label tamamen görünmez
        if (tableSelectDiv) tableSelectDiv.style.display = 'none';
    } else {
        loadOrderTables();
    }
...
function submitOrder() {
    ...
    // Eğer QR'dan masa gelmişse onu kullan
    const masaNo = localStorage.getItem('masaNo');
    if (masaNo) {
        // Masa numarasını ID'ye çevirmek gerekiyor: fetch ile masaları çek
        fetch('/api/tables')
            .then(response => response.json())
            .then(tables => {
                const table = tables.find(t => t.tableNumber == masaNo);
                if (!table) {
                    M.toast({html: 'Masa bulunamadı!'});
                    return;
                }
                sendOrder(table.id, customerName);
            })
            .catch(error => {
                console.error('Masa aranırken hata:', error);
                M.toast({html: 'Masa aranırken bir hata oluştu!'});
            });
    } else {
        // Normal masa seçimi yapıldıysa
        if (!tableId) {
            M.toast({html: 'Lütfen bir masa seçin!'});
            return;
        }
        sendOrder(tableId, customerName);
    }
}

// Asıl sipariş gönderim fonksiyonu
function sendOrder(tableId, customerName) {
    if (!customerName) {
    ...
    .then(response => {
        if (!response.ok) {
            throw new Error('Sipariş oluşturulurken hata!');
        }
        return response.json();
    })
    .then(data => {
        ...
        localStorage.removeItem('masaNo'); // Masa seçimi de temizlensin
        updateCartDisplay();
...
window.addRecommendation = function () {
	...
	const taste = $('#recTaste').val();
	const temperature = $('#recTemp').val();
	const productId = $('#recProductId').val();

	if (!type || !taste || !productId) {
		M.toast({ html: 'Lütfen gerekli tüm alanları doldurun.', classes: 'red' });
		return;
	}
	...
	const recommendation = {
		type: type,
		taste: taste,
		temperature: type === 'içecek' ? temperature : null,
		productId: productId
	};
	...
	fetch('/api/recommendations', {
		...
		.then(res => {
			...
			return res.text(); // JSON değilse text olabilir
		})
		.then(() => {
			M.toast({ html: 'Tavsiye başarıyla eklendi!', classes: 'green' });
			loadRecommendations(); // 🔁 Sayfayı yenilemek yerine sadece listeyi güncelle
		})
		.catch(err => {
			console.error(err);
			M.toast({ html: 'Tavsiye eklenirken hata oluştu.', classes: 'red' });
		});
};


function loadProductListForRecommendations() {
	fetch('/api/products') // Bu endpoint tüm ürünleri döndürmeli
		.then(res => res.json())
		.then(data => {
			const select = document.getElementById('recProductId');
			select.innerHTML = '<option value="" disabled selected>Ürün Seç</option>';
			...
			M.FormSelect.init(select); // Materialize select yenilemesi
		})
		.catch(err => {
	...
	fetch('/api/recommendations')
		...
		.then(recommendations => {
			...
			if (!recommendations || recommendations.length === 0) {
				tbody.innerHTML = '<tr><td colspan="5" class="center-align">Henüz tavsiye eklenmemiş.</td></tr>';
				return;
			}
			...
			recommendations.forEach(rec => {
				...
				tr.innerHTML = `
					...
					<td>${rec.item.name}</td>
					<td>
						<button class="btn red lighten-1 btn-small" type="button" onclick="deleteRecommendation(${rec.id})">
							<i class="material-icons">delete</i>
						</button>
					</td>
				`;
		...
		.catch(err => {
			console.error('Tavsiyeler alınamadı:', err);
			M.toast({ html: 'Tavsiyeler yüklenirken hata oluştu!', classes: 'red' });
		});
}

function deleteRecommendation(id) {
	if (!confirm('Bu tavsiyeyi silmek istediğinize emin misiniz?')) return;
	...
	fetch(`/api/recommendations/${id}`, {
		...
		.then(res => {
			...
			M.toast({ html: 'Tavsiye silindi!', classes: 'green' });
			loadRecommendations(); // listeyi yenile
		})
		.catch(err => {
			console.error(err);
			M.toast({ html: 'Silme işlemi başarısız!', classes: 'red' });
		});
}
